...
A when-in-use grant is normalized as `limited` for this capability.
...
The library reports the system status truthfully and documents this behavior. The status may
downgrade later without any app action.
...
- Request options are a per-request parameter with a default of alert, sound, and badge
