...
Framework-specific information is preserved in concrete, capability-specific status types:
...
Location status types include accuracy authorization in their details. The Always capability
reports a when-in-use grant as `limited`.
...
Executing `manageLimitedSelection` is capability-specific: the photo library picker requires
UIKit presentation bridging, owned by the SwiftUI layer through the
`limitedPhotoLibraryPicker(isPresented:)` modifier, while contact selection management uses the
system's native SwiftUI entry point.
...
Provide an `@Observable` model per capability, built on a domain-level status update stream, that:
...
This is safe because status reads never prompt.
...
1. Status values use concrete per-capability types plus a minimal shared protocol; no generics
2. There is no shared public request protocol; uniform naming provides consistency
3. Notification options are a per-request parameter with a default
4. Location is modeled as two capabilities; the Always capability reports when-in-use as `limited`
5. Recovery is described on each status and executed by the client and SwiftUI layers
...
7. The SwiftUI layer ships an observable model over a domain status stream; no property wrapper

## Remaining open questions

1. Whether the location adapter can adopt session-based system APIs without breaking the one-shot request model
