//
//  LocationWhenInUsePermissionModel.swift
//  Permissionary
//
...
/// Observable status for the when-in-use location capability.
///
...
/// delivered through the location delegate — and exposes explicit
/// refresh and request operations. Creating or observing it never
/// presents a prompt.
@MainActor
@Observable
public final class LocationWhenInUsePermissionModel {
    /// The latest known status, nil until the first read completes.
    public private(set) var status: LocationWhenInUseStatus?

    @ObservationIgnored private let permission: LocationWhenInUsePermission
    @ObservationIgnored private var observation: Task<Void, Never>?

    /// Creates a model observing the given capability.
    ///
    /// - Parameter permission: The capability to observe.
    public init(permission: LocationWhenInUsePermission) {
    ...
    @discardableResult
    public func request() async throws -> LocationWhenInUseStatus {
