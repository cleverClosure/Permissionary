//
//  MicrophonePermissionModel.swift
//  Permissionary
//
...
/// Observable status for the microphone capability.
///
...
@MainActor
@Observable
public final class MicrophonePermissionModel {
    /// The latest known status, nil until the first read completes.
    public private(set) var status: MicrophoneStatus?

    @ObservationIgnored private let permission: MicrophonePermission
    @ObservationIgnored private var observation: Task<Void, Never>?

    /// Creates a model observing the given capability.
    ///
    /// - Parameter permission: The capability to observe.
    public init(permission: MicrophonePermission) {
    ...
    @discardableResult
    public func request() async throws -> MicrophoneStatus {
