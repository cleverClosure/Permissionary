//
//  PhotosAddOnlyPermissionModel.swift
//  Permissionary
//
...
/// Observable status for the photo library add-only capability.
///
...
@MainActor
@Observable
public final class PhotosAddOnlyPermissionModel {
    /// The latest known status, nil until the first read completes.
    public private(set) var status: PhotosAddOnlyStatus?

    @ObservationIgnored private let permission: PhotosAddOnlyPermission
    @ObservationIgnored private var observation: Task<Void, Never>?

    /// Creates a model observing the given capability.
    ///
    /// - Parameter permission: The capability to observe.
    public init(permission: PhotosAddOnlyPermission) {
    ...
    @discardableResult
    public func request() async throws -> PhotosAddOnlyStatus {
